...
import com.trs.modifyData.threadJob.ModifyColumnJob;
...
@Component
@Slf4j
public class HttpMultiProcess {

    @Autowired
    ModifyColumnJob modifyColumnJob;
    ...
    public void MultiProcess(){
        List<TRSRecord> records = modifyColumnJob.fetchData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                modifyColumnJob.updateData(records);
            } else {
    ...
    private class MultiThread implements Runnable{
        ...
        public void run() {
            try {
                ...
                modifyColumnJob.updateData(recordsInner);
            }catch (Exception e){
