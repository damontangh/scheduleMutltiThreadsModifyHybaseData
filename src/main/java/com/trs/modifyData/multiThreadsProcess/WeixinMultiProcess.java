...
import com.trs.modifyData.threadJob.WebsiteAddHybaseid;
import com.trs.modifyData.threadJob.WeixinAddHybaseid;
...
@Component
@Slf4j
public class WeixinMultiProcess {

    @Autowired
    WeixinAddHybaseid weixinAddHybaseid;
    ...
    public void MultiProcess(){
        List<TRSRecord> records = weixinAddHybaseid.fetchData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                weixinAddHybaseid.updateWeixinTrsData(weixinAddHybaseid.processData(records));
            } else {
    ...
    private class MultiThread implements Runnable{
        ...
        public void run() {
            try {
                ...
                weixinAddHybaseid.updateWeixinTrsData(weixinAddHybaseid.processData(recordsInner));
            }catch (Exception e){
