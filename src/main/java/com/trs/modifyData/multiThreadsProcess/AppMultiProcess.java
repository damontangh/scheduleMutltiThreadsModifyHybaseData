...
@Component
@Slf4j
public class AppMultiProcess {

    @Autowired
    AppAddHybaseid appAddHybaseid;
    ...
    public void MultiProcess(){
        List<TRSRecord> records = appAddHybaseid.fetchData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                appAddHybaseid.updateAppTrsData(appAddHybaseid.processData(records));
            } else {
    ...
    private class MultiThread implements Runnable{
        ...
        public void run() {
            try {
                ...
                appAddHybaseid.updateAppTrsData(appAddHybaseid.processData(recordsInner));
            }catch (Exception e){
