...
import com.trs.modifyData.threadJob.WeiboAddHybaseid;
...
@Component
@Slf4j
public class WeiboMultiProcess {

    @Autowired
    WeiboAddHybaseid weiboAddHybaseid;
    ...
    public void MultiProcess(){
        List<TRSRecord> records = weiboAddHybaseid.fetchData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                weiboAddHybaseid.updateWeiboTrsData(weiboAddHybaseid.processData(records));
            } else {
    ...
    private class MultiThread implements Runnable{
        ...
        public void run() {
            try {
                ...
                weiboAddHybaseid.updateWeiboTrsData(weiboAddHybaseid.processData(recordsInner));
            }catch (Exception e){
