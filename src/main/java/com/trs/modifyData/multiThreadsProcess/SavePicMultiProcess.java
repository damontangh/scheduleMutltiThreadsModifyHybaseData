package com.trs.modifyData.multiThreadsProcess;

import com.trs.modifyData.repository.Szb;
import com.trs.modifyData.threadJob.SavePicToMysql;
...
@Component
@Slf4j
public class SavePicMultiProcess {

    @Autowired
    private SavePicToMysql savePicToMysqlJob;
    ...
    public void MultiProcess(){
        List<Szb> records = savePicToMysqlJob.fetchOrigianlData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                savePicToMysqlJob.savePictureToMysqlAndupdateMlfFormalAndOriginal(records);
            } else {
                ...
                for (int i = 1; i <= shardingAmount; i++) {
                    ...
                    List<Szb> partList = records.subList(start,end);
    ...
    private class MultiThread implements Runnable{
        private List<Szb> recordsInner;
        private CountDownLatch startCountInner;
        private CountDownLatch endCountInner;
        //每一个线程负责总list中的一部分，因此传给它的list，就已经是部分list
        public MultiThread(List<Szb> records,CountDownLatch startCount,CountDownLatch endCount){
        ...
        public void run() {
            try {
                ...
                savePicToMysqlJob.savePictureToMysqlAndupdateMlfFormalAndOriginal(recordsInner);
            }catch (Exception e){
