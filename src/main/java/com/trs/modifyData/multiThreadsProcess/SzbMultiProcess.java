...
import com.trs.modifyData.threadJob.AppAddHybaseid;
import com.trs.modifyData.threadJob.SzbAddHybaseid;
...
@Component
@Slf4j
public class SzbMultiProcess {

    @Autowired
    SzbAddHybaseid szbAddHybaseid;
    ...
    public void MultiProcess(){
        List<TRSRecord> records = szbAddHybaseid.fetchData();
        if (records != null && shardingSize != null){
             ...
             */
            if (shardingAmount == 1){
                szbAddHybaseid.updateSzbTrsData(szbAddHybaseid.processData(records));
            } else {
    ...
    private class MultiThread implements Runnable{
        ...
        public void run() {
            try {
                ...
                szbAddHybaseid.updateSzbTrsData(szbAddHybaseid.processData(recordsInner));
            }catch (Exception e){
